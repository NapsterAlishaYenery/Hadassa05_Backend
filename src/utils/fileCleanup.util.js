        ...
        await Promise.all(
            pathsToDelete.map(async (filePath) => {
                try {
                    ...
                    console.log(` Temporal borrado: ${path.basename(absolutePath)}`);
                } catch (err) {
                    // Si el archivo no existe o ya se borró, no queremos que rompa el flujo
                    console.warn(` No se pudo borrar: ${filePath}`);
                }
            })
        );

    } catch (error) {
        console.error("Error en el Utility de limpieza:", error.message);
    }
};
